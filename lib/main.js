...
let checkIfMdn = function(tab) {
  // always hide sidebar, catch error if already hidden
  sidebar.hide().catch(() => {});

  let editURL = /(?:\$(?:edit|translate)|\/docs\/new(?:\?|$))/;
  let viewURL = /(?:docs|Add-ons|Tools|Apps|Marketplace|Firefox_OS|Firefox|Learn|Persona)/;
  let exclude = /\$|docs\/(?:templates|Template(?::|%3A)|all|with-errors|needs-review|top-level|without-parent|tag|files|localization-tag)/;
